...
def get_sync(type, haravan_id):
    ...
    try:
        if type in ['order', 'orders']:
            data = {'tags': 'sync at ' +
            datetime.now().replace(microsecond=0).isoformat(),'updated_at': datetime.now().isoformat()}
            deal_dao.updateDeal(id=haravan_id)
            res['data'] = haravan_service.Order.update(haravan_id, data)
        elif type in ['product', 'products']:
            data = {'barcode': datetime.now().isoformat()}
            res['data'] = haravan_service.Product.update(haravan_id, data)
            if res['data'].get('errors', None):
                res['data'] = haravan_service.Product.updateVariant(
                    haravan_id, data)
        elif type in ['contact', 'contacts']:
            data = {'tags': datetime.now().isoformat()}
...
def check_duplicates(steps=22):
    ...
    while from_id < to_id:
        print(from_id)
        try:
            data = bitrix24_service.Deal.get(from_id)
            if data:
                ...
                if ha_id:
                    ...
                    if dao.get('data', None):
                        ...
                        if bx_id:
                            ...
                            if not from_id == bx_id:
                                # Nếu id khác bx_id xoá Deal=id
                                bitrix24_service.Deal.delete(from_id)
                    else:
                        # Nếu không có ha_id thêm mới record tbl_deal_order
                        print(deal_dao.addNewDeal(ha_id, from_id, None, None))
        except Exception as err:
            # retry_dao.insertRetryJobRecord(bitrix24_id=from_id)
